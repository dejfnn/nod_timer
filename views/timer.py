...
def _render_timer_section() -> None:
    """Render the timer control section at the top of the page."""
    st.subheader("Timer")
    ...
    col_desc, col_proj = st.columns([3, 2])
    with col_desc:
    ...
    if is_running:
        ...
        if st.button("Start", key="start_btn", type="primary", use_container_width=True):
            start_timer(
...
def _render_manual_entry() -> None:
    """Render the manual time entry form."""
        ...
        with st.form(key="manual_entry_form"):
            ...
            with col_end:
                end_time = st.time_input(
                    "End time",
                    value=(datetime.now().replace(minute=0, second=0) + timedelta(hours=1)).time(),
                    key="manual_end",
                )
