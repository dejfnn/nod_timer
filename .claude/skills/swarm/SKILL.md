---
name: swarm
description: "Orchestrated multi-agent implementation using Tech Lead + Developer pattern. Use when implementing phased plans. Triggers: /swarm, 'implement the plan', 'run phased implementation', 'execute plan phases'. Spawns Developer agents for each phase, reviews PRs, handles retries and escalation."
---
...
4. **Integration points** - new modules are wired into the application entry point, config vars documented
...
- Forget to wire new modules into the application

## Rules
1. Follow CLAUDE.md project standards (if present)
...
6. Detect project language/framework and use appropriate tooling (test runners, linters, etc.)
...
- [ ] All tests pass (use the project's test runner)
- [ ] New modules are wired into the application entry point
- [ ] New config vars are documented in .env.example (if applicable)
...
- Any file is a stub (empty class, no-op functions, placeholder implementations)
...
|-----------|----------|----------|
| "Users can register" | auth/service.py:register() | test passes |
| "Unique email constraint" | migration has UNIQUE KEY | confirmed |
| "Role-based access" | middleware uses require_role() | test passes |
```
...
```bash
# Detect and run the project's test suite for this phase
# Examples: pytest, npm test, go test, cargo test, etc.
<project-test-command> -v
```

**FAIL if:**
- Any test fails
- Test count is significantly lower than specified in plan
- Tests are trivial (e.g., `assert True`, `expect(true).toBe(true)`)
...
```bash
# Is the new module wired into the application entry point?
# (router registered, plugin loaded, service injected, etc.)

# Are new config vars documented?
# Check .env.example or equivalent

# Do data models match their migrations/schema? (if applicable)
```

**FAIL if:**
- New module not wired in (endpoints unreachable, service not available)
- Config vars missing from documentation
- Schema mismatches between code and migrations

### 4.5 Code Quality Check

1. **No hardcoded values** - configuration belongs in config files
2. **No silent defaults** - missing required config = startup failure
3. **Consistent ID types** - follow existing project conventions
...
### File Inventory: PASS
- [x] All N files created
- [x] No stubs or placeholders

### Acceptance Criteria: PASS
| Criterion | Status |
|-----------|--------|
| Criterion 1 | Verified in file:line |
| Criterion 2 | Test passes |
...
1. **Acceptance Criteria #3 not implemented**
   - "Feature X calculates correctly"
   - No implementation found
   - Required: Add calculate_x() method

2. **Missing test**
   - "Tests Required" lists "Edge case handling" but no such test exists
